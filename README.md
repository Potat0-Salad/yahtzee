...
App runs at **http://localhost:5190** (fixed via `server.port` in `vite.config.ts`; `strictPort` is set so it fails loudly instead of drifting to another port if 5190 is taken). Backend API at `http://localhost:8000`.
